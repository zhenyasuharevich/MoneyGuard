...
enum MainCategoriesCellType: Int {
  case addCategory = 0
  case category = 1
  
  static func getCellType(for indexPath: IndexPath, arrayCount: Int) -> MainCategoriesCellType {
...
final class MainCategoriesCell: UICollectionViewCell {
  ...
  private var state: MainCategoriesCellType = .category {
    didSet {
      switch state {
      case .addCategory:
  ...
  func setState(state: MainCategoriesCellType) {
    self.state = state
  }
